...
export async function syncOfflineQueues() {
  if (!navigator.onLine) return { inspections: 0, photos: 0, remainingInspections: readQueue(INSPECTION_QUEUE_KEY).length, remainingPhotos: readQueue(PHOTO_QUEUE_KEY).length };
