    ...
    try {
      ...
      setMessage(
        `Generated ${result.sections_created} section(s) of ${result.section_length_m} m ` +
          `(total ${result.total_length_km} km). Sections show below; refresh Overview map to see them.`,
      );
  ...
  return (
    <section className="action-panel road-register" aria-label="Road register">
      ...
      {!loading && roads.length === 0 && (
        <EmptyState title="No roads yet" tone="info">
          <p>
            Create a road (code + name), optionally draw geometry, then
            <strong> Generate sections</strong> (500 m). Demo seed: GitHub Action
