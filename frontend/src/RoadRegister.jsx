...
export default function RoadRegister() {
  ...
  async function handleSubmit(e) {
    ...
    if (form.geometry_wkt.trim()) {
      payload.geometry_wkt = form.geometry_wkt.trim();
    }
    try {
      if (editingId) {
  ...
  async function handleGenerateSections(replace = false) {
    ...
    if (
      replace &&
      !window.confirm("Replace all existing sections for this road?")
    ) {
      return;
    }
    setSaving(true);
        ...
        <table>
          ...
          <tbody>
            {roads.map((road) => (
              <tr
                key={road.road_id}
                className={selectedId === road.road_id ? "row-selected" : ""}
              >
                <td>{road.road_code}</td>
                <td>{road.road_name}</td>
                <td>{road.road_class || "—"}</td>
                <td>{road.total_length_km ?? "—"}</td>
                <td>{road.status}</td>
                <td className="row-actions">
                  <button type="button" onClick={() => startEdit(road)}>
                    Edit
                  </button>
                  <button type="button" onClick={() => loadSections(road.road_id)}>
                    Sections
                  </button>
                  {road.status !== "archived" && (
                    <button type="button" onClick={() => handleArchive(road)}>
                      Archive
                    </button>
                  )}
                </td>
        ...
        <div className="section-panel">
          ...
          <div className="filter-bar">
            ...
            <button
              type="button"
              disabled={saving}
              onClick={() => handleGenerateSections(true)}
            >
              Regenerate (replace)
            </button>
          </div>
              ...
              <table>
                ...
                <tbody>
                  ...
                  {sections.length === 0 && (
                    <tr>
                      <td colSpan={6}>
                        No sections yet. Set length or geometry, then generate.
                      </td>
                    </tr>
                  )}
                </tbody>
