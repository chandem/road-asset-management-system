...
export default function OfflineInspectionQueue({ sections = [] }) {
  ...
  async function refresh() {
    try {
      setQueue(await getPendingInspections());
    } catch (error) {
      setMessage(`Offline storage error: ${error.message}`);
    }
  }
  ...
  async function syncQueue() {
    ...
    try {
      ...
      await refresh();
      const conflictNote =
        result.conflicts || result.conflictInspections
          ? ` ${result.conflictInspections || result.conflicts || 0} conflict(s) need review.`
          : "";
      setMessage(
        `Synced ${result.inspections} inspection(s), ${result.photos} photo(s). ${result.remainingInspections} still pending.${conflictNote}`,
      );
    } catch (error) {
  ...
  async function saveOffline(event) {
    ...
    if (!sectionId) {
      setMessage("Select a road section.");
      return;
    }
    const section = sections.find(
      (item) => String(item.section_id) === String(sectionId),
    );
    const clientId =
    ...
    try {
      ...
      await refresh();
      setMessage(
        `Inspection for ${record.section_code || `section #${record.section_id}`} saved offline.`,
      );
      setConditionRating("");
      setWeather("");
      setNotes("");
    } catch (error) {
      setMessage(`Could not save inspection offline: ${error.message}`);
    }
  }

  async function removeRecord(clientId) {
    await deleteInspection(clientId);
    await refresh();
  }

  async function retryRecord(item) {
    await putInspection({
      ...item,
      status: "pending",
      attempts: 0,
  ...
  async function clearQueue() {
    if (
      !queue.length ||
      !window.confirm(`Delete ${queue.length} offline inspection record(s)?`)
    ) {
      return;
    }
    for (const item of queue) await deleteInspection(item.client_id);
    await refresh();
    setMessage("Offline inspection queue cleared.");
  }
  ...
  function exportQueue() {
    ...
    link.href = url;
    link.download = `rams-inspections-${new Date().toISOString().slice(0, 10)}.json`;
    link.click();
  ...
  return (
    <section className="form-panel offline-inspection-panel">
      <div className="form-header">
        <div>
          <h2>Offline Inspection Capture</h2>
          <p>
            Capture inspections without internet. Records persist on this device
            and sync when connectivity returns. Conflicts stop auto-retry until
            you Retry or Discard.
          </p>
        </div>
      </div>

      <div className="offline-queue-summary" aria-label="Queue counts">
        <div className="card">
          <span>Pending</span>
      ...
      <form onSubmit={saveOffline}>
            ...
            >
              ...
              {sections.map((section) => (
                <option key={section.section_id} value={section.section_id}>
                  {section.section_code} ({section.start_chainage}–
                  {section.end_chainage} km)
                </option>
              ))}
            </select>
          </label>
        ...
        <div className="form-footer">
          <button type="submit">Save Inspection Offline</button>
          <button
            type="button"
            onClick={syncQueue}
      ...
      <div className="offline-inspection-queue">
        <div className="form-header">
          <div>
            <h3>Device queue</h3>
            <p>
              {queue.length
                ? "Stored on this device until synced or discarded."
                : "No offline inspections queued."}
            </p>
          </div>
              ...
              <tbody>
                {queue.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <td>{item.section_code || `#${item.section_id}`}</td>
                      <td>{item.inspection_date}</td>
                      <td>{item.condition_rating ?? "—"}</td>
                      <td>
                      ...
                      <td className="offline-error-cell">
                        {item.last_error || "—"}
                      </td>
                      <td>
                        {item.captured_at
                      ...
                      <td>
                        <div className="offline-actions">
                          {st === "conflict" && (
                            <button type="button" onClick={() => retryRecord(item)}>
                              Retry
                            </button>
                          )}
                          <button
                            type="button"
                            onClick={() => removeRecord(item.client_id)}
                          >
                            Discard
                          </button>
                        </div>
