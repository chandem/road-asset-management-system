...
export default function MaintenancePanel({ roads = [], sections = [] }) {
  ...
  const completed = activities.filter((a) => String(a.status).toLowerCase() === "completed").length;
  const estimatedTotal = activities.reduce(
    (sum, a) => sum + (a.estimated_cost != null ? Number(a.estimated_cost) : 0),
    0,
  );
  const actualTotal = activities.reduce(
    (sum, a) => sum + (a.actual_cost != null ? Number(a.actual_cost) : 0),
    0,
  );
    ...
    <section className="form-panel maintenance-panel">
      <div className="form-header">
        <div>
          ...
          <p>Plan, record and track road maintenance activities and costs.</p>
        </div>
      </div>
      ...
      {!roadId && (
        <EmptyState title="Select a road to manage maintenance" icon="🛠️" tone="info">
          <p>Choose a road above to add activities and review maintenance history.</p>
        </EmptyState>
      )}
        ...
        <>
          <div className="insight-kpi-grid compact">
            ...
            <div className="insight-kpi">
              ...
              <strong>
                {loading ? "…" : `${estimatedTotal.toLocaleString()} ETB`}
              </strong>
              <small>planned</small>
            </div>
            <div className="insight-kpi">
              ...
              <strong>
                {loading ? "…" : `${actualTotal.toLocaleString()} ETB`}
              </strong>
              <small>spent</small>
            </div>
          </div>

          <form onSubmit={submit} className="grouped-form">
            <div className="form-section">
              ...
              >
                ...
                <span aria-hidden="true">{openSections.schedule ? "▾" : "▸"}</span>
              </button>
            ...
            <div className="form-section">
              <button
                ...
                aria-expanded={openSections.cost}
              >
                <span>Cost</span>
                <span aria-hidden="true">{openSections.cost ? "▾" : "▸"}</span>
              </button>
              {openSections.cost && (
                <div className="form-grid">
                  <label>
            ...
            <div className="form-section">
              ...
              >
                <span>Description & contractor</span>
                <span aria-hidden="true">{openSections.details ? "▾" : "▸"}</span>
              </button>
            ...
            <div className="form-footer">
              <button type="submit" className="primary" disabled={saving}>
                {saving ? "Saving…" : "+ Add Maintenance Activity"}
              </button>
          ...
          <div className="ai-results">
            ...
            {!loading && activities.length === 0 && (
              <p className="muted-note">No maintenance activities recorded for this road.</p>
            )}
            {activities.map((a) => (
              <div className="ai-result" key={a.maintenance_id}>
                ...
                <span>
                  {a.status} · {a.priority || "No priority"}
                </span>
                <small>
                  {a.planned_date ? `Planned ${a.planned_date}` : "No planned date"}
                  {a.estimated_cost != null
                    ? ` · Est. ${Number(a.estimated_cost).toLocaleString()} ETB`
                    : ""}
