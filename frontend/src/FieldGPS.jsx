...
export default function FieldGPS() {
  ...
  function saveCurrentOffline() {
    ...
    setOfflineQueue((current) => [...current, {
      ...
      road_id: gpsMatch?.road_id ?? roadId || null,
      section_id: gpsMatch?.section_id ?? sectionId || null,
      chainage_km: gpsMatch?.chainage_km ?? null,
      source: "field-gps-offline",
      synced: false,
    }]);
    setError("");
  }
  ...
  function exportOfflineQueue() {
    ...
    link.href = url;
    link.download = `rams-gps-${new Date().toISOString().slice(0, 10)}.json`;
    link.click();
  ...
  return (
    <section className="form-panel field-gps-panel">
      <div className="form-header">
        <div><h2>📍 Field GPS Capture</h2><p>Capture GPS, match road/section/chainage, and store field points offline.</p></div>
        <div className="actions"><button type="button" onClick={captureOnce}>Capture GPS</button><button type="button" onClick={() => setWatching((v) => !v)}>{watching ? "Stop Tracking" : "Start Tracking"}</button></div>
      </div>
      ...
      <div className="field-gps-grid">
        <div className="gps-details">
          ...
          <div className="card"><span>Offline queue</span><strong>{offlineQueue.length} point{offlineQueue.length === 1 ? "" : "s"}</strong></div>
        </div>
        <MapContainer center={position ? [position.latitude, position.longitude] : DEFAULT_CENTER} zoom={position ? 15 : 7} className="field-gps-map">
          ...
          {position && <><CircleMarker center={[position.latitude, position.longitude]} radius={9} pathOptions={{ fillOpacity: 0.85 }} /><Recenter position={position} /></>}
          {gpsMatch?.matched && gpsMatch.projected_point?.coordinates && <CircleMarker center={[gpsMatch.projected_point.coordinates[1], gpsMatch.projected_point.coordinates[0]]} radius={6} pathOptions={{ fillOpacity: 0.7 }} />}
          {nearestPoint && <CircleMarker center={[Number(nearestPoint.latitude), Number(nearestPoint.longitude)]} radius={5} pathOptions={{ fillOpacity: 0.55 }} />}
        </MapContainer>
      </div>

      <div className="actions offline-actions">
        <button type="button" onClick={saveCurrentOffline} disabled={!position}>Save GPS Point Offline</button>
        <button type="button" onClick={exportOfflineQueue} disabled={!offlineQueue.length}>Export Offline Data</button>
        <button type="button" onClick={clearOfflineQueue} disabled={!offlineQueue.length}>Clear Queue</button>
      </div>
      ...
      <small>Offline records remain in this browser until exported or cleared. Server synchronization will be added next.</small>
