...
export default function OfflineDefectQueue({ sections = [] }) {
  ...
  async function saveOffline(event) {
    ...
    if (!inspectionClientId && !inspections.some((item) => item.synced && item.inspection_id)) {
      setMessage("Create or select an inspection before recording a defect.");
      return;
    }
    if (!defectType.trim()) {
  ...
  return (
    <section className="form-panel offline-inspection-panel">
      <div className="form-header">
        <div><h2>⚠️ Offline Defect Capture</h2><p>Record road defects in the field without internet. Defects are synchronized after their parent inspection.</p></div>
        <div className="card"><span>Queued</span><strong>{queue.length}</strong></div>
      </div>
      <form onSubmit={saveOffline}>
        <div className="form-grid">
          <label>Inspection
            <select value={inspectionClientId} onChange={(event) => setInspectionClientId(event.target.value)}>
              <option value="">Select offline inspection</option>
              {inspections.map((item) => <option key={item.client_id} value={item.client_id}>{item.section_code || `Section #${item.section_id}`} · {item.inspection_date}{item.synced ? " · synced" : " · pending"}</option>)}
            </select>
          </label>
          <label>Road Section
            <select value={sectionId} onChange={(event) => setSectionId(event.target.value)}>
              <option value="">Use inspection section</option>
              {sections.map((section) => <option key={section.section_id} value={section.section_id}>{section.section_code} ({section.start_chainage}–{section.end_chainage} km)</option>)}
            </select>
          </label>
