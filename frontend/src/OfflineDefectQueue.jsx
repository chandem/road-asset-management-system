...
export default function OfflineDefectQueue({ sections = [] }) {
  ...
  async function refresh() {
    try {
      const [allInspections, allDefects] = await Promise.all([
        getInspections(),
        getDefects(),
      ]);
  ...
  async function saveOffline(event) {
    ...
    if (!inspectionClientId) {
      setMessage("Select an inspection before recording a defect.");
      return;
    }
    if (!defectType.trim()) {
      setMessage("Enter the defect type.");
      return;
    }
    const selectedInspection = inspections.find(
      (item) => item.client_id === inspectionClientId,
    );
    if (!selectedInspection) {
      setMessage("Selected inspection is no longer available on this device.");
      return;
    }
    const selectedSection = sections.find(
      (item) =>
        String(item.section_id) ===
        String(sectionId || selectedInspection.section_id),
    );
    const clientId =
    ...
    const record = {
      ...
      inspection_id: selectedInspection.synced
        ? selectedInspection.inspection_id
        : null,
      section_id: sectionId
        ? Number(sectionId)
        : selectedInspection.section_id ?? null,
      ...
      depth_mm: depthMm === "" ? null : Number(depthMm),
    ...
    try {
      await putDefect(record);
      await refresh();
      setMessage(
        `Defect saved offline for ${record.section_code || "the selected inspection"}.`,
      );
      setDefectType("");
      setChainageKm("");
      setLengthM("");
      ...
      setMessage(`Could not save defect offline: ${error.message}`);
    }
  }
  ...
  async function syncQueue() {
    ...
    try {
      ...
      await refresh();
      const conflictNote =
        result.conflictDefects || result.conflicts
          ? ` ${result.conflictDefects || result.conflicts || 0} conflict(s) need review.`
          : "";
      setMessage(
        `Synced ${result.inspections} inspection(s), ${result.defects} defect(s), ${result.photos} photo(s). ${result.remainingDefects} defect(s) still pending.${conflictNote}`,
      );
    } catch (error) {
  ...
  return (
    <section className="form-panel offline-inspection-panel">
      <div className="form-header">
        <div>
          <h2>Offline Defect Capture</h2>
          <p>
            Record road defects without internet. Each defect stays linked to its
            parent inspection. Conflicts require Retry or Discard.
          </p>
        </div>
      </div>
      ...
      <form onSubmit={saveOffline}>
        <div className="form-grid">
          ...
          <label>
            ...
            <input
              ...
              step="0.001"
              value={chainageKm}
              onChange={(event) => setChainageKm(event.target.value)}
            />
          </label>
          <label>
            Length (m)
        ...
        <div className="form-footer">
          <button type="submit">Save Defect Offline</button>
          <button
            type="button"
            onClick={syncQueue}
                    ...
                    <td>
                      <div className="offline-actions">
                        ...
                        <button
                          type="button"
                          onClick={async () => {
                            await deleteDefect(item.client_id);
                            await refresh();
                          }}
                        >
                          Discard
