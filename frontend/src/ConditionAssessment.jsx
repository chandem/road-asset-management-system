...
export default function ConditionAssessment() {
  ...
  useEffect(() => {
    getRoads().then(setRoads).catch((err) => setError(err.message));
  }, []);
  ...
  return <section className="panel">
    ...
    <div className="button-row">
      ...
      <button type="button" onClick={assess} disabled={!roadId}>Assess road</button>
    </div>
    {error && <div className="auth-error">{error}</div>}
    {report && <>
      <p>Average condition score: <strong>{report.average_score}</strong> / 100 · {report.section_count} sections</p>
      <div className="table-wrap">
        <table>
          <thead><tr><th>Section</th><th>Chainage</th><th>Score</th><th>Condition</th><th>Priority</th><th>Recommendation</th><th>Maintenance</th></tr></thead>
          <tbody>
            {report.items.map((item) => <tr key={item.section_id}>
              ...
              <td>
                {created[item.section_id]
                  ? <span>Created #{created[item.section_id]}</span>
                  : <button
                      type="button"
